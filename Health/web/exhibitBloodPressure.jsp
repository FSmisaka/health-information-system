...
<%
    ...
    ArrayList<HealthData> record = DAOFactory.getIBloodDAOInstance().dataTrend(id);

    BloodPressure data0 = null;
    try {
        data0 = (BloodPressure)record.get(0);
    } catch (Exception ex) {
        data0 = new BloodPressure();
        ex.printStackTrace();
    }
    ...
    while (iter.hasNext()) {
        BloodPressure data = (BloodPressure)iter.next();
...
<div class="content" style="text-align: left;">
    <h3>最新记录</h3>
    血&nbsp&nbsp压: <%=(int)data0.getData()%> mmHg
    <br/>
    记录时间: <%=data0.getDate()%>
    <br/>
</div>
...
<script>
    ...
    // 调用通用函数绘制图表
    drawLineChart('healthDataChart', dates, values, '血压', '日期', '值');
</script>
